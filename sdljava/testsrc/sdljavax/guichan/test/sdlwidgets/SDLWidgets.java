...
package sdljavax.guichan.test.sdlwidgets;
...
public class SDLWidgets {
	...
	private SDLGraphics		graphics;								// Graphics driver
	private SDLImageLoader	imageLoader;							// For loading images

	private GUI				gui;									// A Gui object - binds it all
	// together
	...
	private ScrollArea		textBoxScrollArea;						// Scroll area for the text box
	...
	public void init() throws SDLException, GUIException {
		...
		screen = SDLVideo.setVideoMode(640, 480, 32, SDLVideo.SDL_HWSURFACE);
		// We want unicode
	...
	private boolean checkInput() throws SDLException {
		boolean bHadEvent = false;
		/*
		 * Poll SDL events
		 */
	...
	public void run() throws SDLException, GUIException {
		while (running) {
			// Poll input and react on messages
			if (checkInput()) {
	...
	public static void main(String[] args) {
		try {
			SDLWidgets demo = new SDLWidgets();
			demo.init();
			demo.run();
			demo.delete();
		} catch (Exception e) {
